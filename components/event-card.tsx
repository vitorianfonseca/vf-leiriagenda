...
export function EventCard({ id, title, date, time, location, image, category }: EventCardProps) {
  ...
  useEffect(() => {
    if (isAuthenticated) setIsFavorited(isFavorite(id))
  ...
  return (
    <Card className="group hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden border border-border/60 bg-card">
      <div className="relative overflow-hidden">
        ...
        >
          <Heart className={`h-4 w-4 transition-colors ${isAuthenticated && isFavorited ? "text-red-500 fill-red-500" : "text-primary/60 hover:text-red-400"}`} />
        </button>
      </div>
